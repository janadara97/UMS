      ...
      <v-card>
        <v-layout row>
          <v-flex class="primary white--text text-center">
            <v-card-title>
              <span class="text-h5 pa-5">Edit Module</span>
            </v-card-title>
          </v-flex>
        </v-layout>
          ...
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field
                  label="Module Name*"
                  :rules="[minLength('Name', 5), maxLength('Name', 50)]"
        ...
        <v-card-actions>
          ...
          <v-btn :loading="loading" color="green white--text" @click="updateModule"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-col>
...
export default {
  ...
  methods: {
   async updateModule() {
    ...
    await userService.updateModule(this.item).then((response)=>{
